# ONE-PAGER — Defect Notice Decision, 6840 Stonington Rd

> ⚠️ **Strategy analysis, not legal advice — review with your agent and an Ohio attorney before signing.**

## THE CLOCK

| Date | What happens |
|---|---|
| **Mon 08/10 EOD** | Respond or we are **deemed to have agreed to ALL repairs** as written |
| **Wed 08/12 EOD** | If we counter and there's no mutual **signed** agreement → contract **auto-terminates** |
| **Fri 08/28** | Closing (re-inspection ~08/25; loan approval ~08/21) |

## THE DECISION

One phone call Saturday decides the branch: **Amy calls Zach West** — do the buyers want money or the repairs done?

| Zach says… | We deliver Monday |
|---|---|
| Money is fine | Radon fixed pre-closing (licensed, retest <4.0) + **$5,500** credit for roof/gutters — option (d) + amendment |
| They want repairs done | **(b)-amended**: we do everything, with scope pinned (RS-2 three locations; licensed radon) — zero termination risk |
| Call ambiguous / doesn't happen | **(b)-amended** (safe default) · If radon can't be booked by Mon → pure credit **~$7,500** |

## THE NUMBERS

- **Their ask, all-in:** ~$3,800–$10,200; most likely **$5,500–$7,700** (roof pkg $2,500–$7,500 + gutters $300–$800 + radon $1,000–$2,000)
- **Walking away (BATNA):** **−$12K to −$30K** vs. closing, **plus $5K–$13K carry**, through a separation, with full disclosure on relist
- **The floor:** even silence (= all repairs, ~$5.5–10K worst case) still beats walking. Every branch on this page wins; only delay loses.

## 3 REASONS THIS WORKS

1. **The buyers are committed** — over-ask, $10K earnest, $10K appraisal-gap, waived foundation/panel/HVAC. They want the house, not a fight.
2. **Radon demonstrably fits** — licensed install 08/14–08/18, retest result 08/18–08/22, done before the 08/25 re-inspection with 3–7 days margin.
3. **A credit is a cap, not a bid** — fixed dollar exposure, far under the lender's ~$35,100 allowance, nothing owed after closing.

## 3 RISKS

1. **Buyers may want repairs, not money** — they wrote "Seller to have roofing contractor." That's exactly why the Saturday call gates the choice.
2. **Appraiser flags the roof** (~08/14–21) → "subject to repairs" forces pre-closing work regardless; the (b)-amended branch hedges this.
3. **Two-signer bottleneck Sunday** — any counter must be signed by both Jeff and Olivia before Monday; an unsigned packet on 08/12 kills the contract.

## WHAT WE NEED FROM AMY

- **The Saturday call to Zach** (the gate) — plus lender confirmation the credit fits buyers' actual closing costs
- Form prep + amendment drafting from [DRAFT_RESPONSE_LANGUAGE.md](DRAFT_RESPONSE_LANGUAGE.md); deliver Monday **afternoon**
- **One real roofer quote ASAP** — it supersedes every research band above
- Radon installer numbers from her network (2–3 ODH-licensed firms to call Saturday)

## WHAT JEFF & OLIVIA MUST DO

- **Saturday alignment call:** agree on the branch logic and the numbers ($5,500 / $7,500) before the gate result lands
- **Be signable Sunday:** e-signature packet ready by noon; both signatures done Sunday — phone + e-sign only, no meeting needed
