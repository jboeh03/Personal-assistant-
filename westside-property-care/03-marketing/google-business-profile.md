...
**Human action required:** creation and verification are on `05-playbook/human-checklist.md` #1 — no agent can do this
...
Not rank for the region.** GBP does not generate demand for this business — `CANON.md` §5 is
...
runs days to several weeks (`human-checklist.md` #1). Start it before the website exists, before
the first door hanger prints, before Track A or Track B are resolved. It gates nothing else, and
nothing else should gate it.
...
| Field | Value | Note |
|---|---|---|
| Business name | **Westside Property Care** | Must match the approved legal name exactly before the profile is claimed publicly — `brand-brief.md` §1. If the Ohio LLC filing lands on the fallback name (Westside Property Care of Cincinnati LLC), the *public-facing* GBP name stays "Westside Property Care." |
| Business type | **Service-area business** — no storefront, no public address | `human-checklist.md` #1 |
...
| Website | `[URL]` | Not yet decided — see `gtm-plan.md` §8. Leave the field blank rather than filling it with a placeholder domain if the site isn't live yet; an unreachable URL on a verified listing is worse than no URL. |
| Hours | By appointment / does not apply in the traditional sense | Routine visits run only Monday and Friday of weeks 1 and 3 (`operating-model.md` §3) and this is not a walk-in business. If GBP requires hours, list them as the days he's reachable by phone/text for new inquiries, not a storefront schedule. |
...
| Pool cleaning service | **Not pool service.** `service-catalog.md` §4.1 is the most important exclusion in the business — no water chemistry, no chemicals, no vacuuming, no filter work. This category would set an expectation the agreement explicitly disclaims. |
...
project prices appear here** — the membership price is never listed on GBP, because
`messaging.md`'s own surface table routes the membership conversation to a call or message, not
to a price a stranger reads off a search result.

| Service name | Description | Price shown |
|---|---|---|
| Outdoor living property care membership | Twice-monthly care for a pool deck, patio, and outdoor kitchen on the west side, with a written report after every visit. Six properties, maximum. | None — "call or message to see if a spot's open" |
...
are explicitly not customer-facing (`pricing.md` §5.3, `voice.md` §8 item 19).

---

## 5. Business description

**750-character limit.** Opens with the approved short form from `brand-brief.md` §2 verbatim,
then extends with geography and the two published prices, per the lead order in `messaging.md`
§3's GBP row.
...
> a lawn service and not a pool service — I don't mow, and I don't touch pool chemistry. What I
> do is skim, wipe, blow clean, and write down what I find, twice a month, on your route day.
>
> I also do grill and outdoor kitchen deep cleans starting at $249, and window well cleanouts
> starting at $49 during a scheduled visit. Call or text `[PHONE]`.

(Character count comes in under 750 with the placeholder resolved to a real number; recount
once `[PHONE]` is filled in.)

---

## 6. Photo plan

Tied directly to the shot list in `05-playbook/human-checklist.md` #8. **No stock photography,
ever, as a final answer** (`CANON.md` §9.3, `visual-direction.md` §8). Until real photographs
exist, the profile ships with the logo/mark only rather than a placeholder image — an empty
gallery is honest; a stock patio is not.

| GBP photo slot | Source shot (per `human-checklist.md` #8) | Caption / alt-text approach |
|---|---|---|
| Logo | The WPC513 mark (`visual-direction.md` §7) — typographic, no icon | — |
| Cover photo | The wide shot of a whole outdoor living area looking genuinely ready to use | Name the space and the season: "Pool deck and patio, blown clean, [season]." |
| "At work" photos | Him working — hands, tools, actual effort, not posed | "Skimmer basket lifted out" or equivalent — name the object per `voice.md`'s alt-text rule, never "pool maintenance" as a caption |
| Additional photos | The pool skimmed and clean with both baskets emptied; the outdoor kitchen before/after; the truck and equipment; the dormant-season shot (covered furniture, salted flagstone, bare trees) | Same rule — name the object, locate it |

**Do not upload a shot until it's real.** A profile with three real photos looks more credible
than one padded to twelve with anything staged or sourced. The dormant-season shot is
non-negotiable once it exists — without it the profile reads as a summer-only business seven
months of the year (`visual-direction.md` §8, shot 5).

---

## 7. Posting cadence

This is not a content-volume channel — the system prompt for this whole campaign says premium
local clients are won by proximity, proof, and referral, not content volume, and a six-client
business posting weekly generic updates would read as exactly the wrong kind of business. Post
only around real, verifiable events:

| Trigger | Cadence |
|---|---|
| Season transition | Twice a year — April 1 (Green Season starts) and November 1 (Dormant Season starts), each naming the actual scope shift per `service-catalog.md` §§2–3 |
...
`human-checklist.md` #1 has the checklist. Restated here because it's the reason this whole file
starts on day one:
...
- `05-playbook/human-checklist.md` #1, #8 — the human tasks this file cannot do
