...
>    contacted.
> 2. **A lawful consent basis for every channel used**, confirmed per-channel, not as a blanket
>    assumption:
>    - **Phone calls and physical mail** carry the lowest consent burden of the channels in this
>      file, but a live sales call to numbers on the National Do Not Call Registry still carries
>      real restrictions — check the registry, or default to mail and email as the first touch,
>      before calling anyone who hasn't already invited a call.
>    - **Email** is governed by CAN-SPAM: commercial email is generally permitted without prior
>      opt-in, but every message must carry accurate sender identification, a real physical
>      address, and a working, honored opt-out. §5 below builds those requirements into the
>      templates themselves, not just into this policy note.
...
>    number, no ambiguity about who is contacting them or why.
> 5. **The owner has personally reviewed and approved this specific use of his family's business
>    relationship**, separate from the legal questions above. This is his brother's customer
>    list and his own family's goodwill; a legal green light is necessary but not sufficient.
>
> **This file is not legal advice**, in the same sense `service-catalog.md` §4 disclaims legal
> drafting. TCPA and CAN-SPAM rules have changed as recently as April 2025 and continue to
> evolve. Confirm current requirements with counsel or a compliance resource before sending
> anything, not just before this file was written.
>
> **Everything past this banner is written and ready.** The day access and consent are both
> confirmed, this file executes without a rewrite. Until then, it is a plan, not a task list.
...
- **Attribute 1 — a pool, an outdoor kitchen, or both.** Every one of them has an outdoor
  ...
  first place.
- **Attribute 2 — already pays someone to maintain something.** They already paid roughly $249
...
- **Serviced in the last 12–18 months:** warmer — the address, the equipment, and the
  relationship are all more likely to still be current.
- **Repeat TSGC customers** (more than one cleaning): a stronger signal of real, ongoing
  investment in the outdoor kitchen than a single one-off job.
- Older, single-visit records go last within their tier, not excluded — just lower priority for
  the first pass.
...
## 4. Channel order, and why

Consent risk, not marketing preference, sets this order.

1. **Phone call or a personal letter first.** Lowest consent burden of the channels available,
   and the most natural fit for someone he already has a relationship with — a call from someone
   whose truck you recognize reads very differently from a call from a stranger. Check the
   National Do Not Call Registry status of a number before calling if there's any doubt about
   whether the existing relationship covers it.
2. **Email second**, for anyone not reached by phone, or as a leave-behind after a call. Governed
   by CAN-SPAM's opt-out model (§ banner above) — permitted with accurate sender ID, a physical
   address, and a working unsubscribe, all built into the template in §5.
3. **SMS last, and only for people who ask for it.** Never a first-touch channel to the raw list.
   A reply to the call or email that says "just text me" or a checked box on a reply form is
   what turns a name into an SMS-eligible contact — nothing else does.

---

## 5. The founding-member sequence

All copy below passes `voice.md` §8. Every template carries its own consent-basis and opt-out
language, not just a policy note elsewhere in this file — that's the actual requirement, not
just good practice.

### Step 1 (Week 1) — Phone call, or a personal letter for names without a working number
...
Sent **only** to a contact who has replied to Step 1–3 asking to be texted, or checked an opt-in
box on a reply form. Never sent cold to the list.
...
## 6. What happens to Tier 3, and to anyone who opts out

- **Tier 3 names are never entered into this sequence.** See §3 — this campaign does not target
  excluded geography, warm list or not.
- **Any opt-out, on any channel, is honored the same day it's received where practical, and
