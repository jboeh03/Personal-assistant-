...
$329/month off a door hanger"). No price appears anywhere on this piece — a door-hanger reader is
...
| Color | `--paper` ground, `--ink` text, `--copper` used only if the piece is printed in more than one color and only on the phone number or the mark rule — per `visual-direction.md` §2.5, copper is the only action color and nothing else on the piece should carry it |
...
| Typography | Front headline in Fraunces (per `visual-direction.md` §3); body and the `[OWNER FIRST NAME]` / `[PHONE]` / WPC513 line in IBM Plex Sans; sentence case throughout, no exclamation marks, no glyphs |
